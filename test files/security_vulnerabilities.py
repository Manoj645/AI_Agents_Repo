...
if __name__ == "__main__":
    ...
    # 1) Command injection risk with unsanitized input (security)
    def dangerous_shell(user_input: str):
        # VULNERABLE: Directly interpolating into shell command
        return subprocess.check_output(f"echo {user_input} && ls -la", shell=True)

    # 2) Hardcoded secrets and magic numbers (Rules 7)
    API_SECRET = "supersecretapikey123456"  # hardcoded secret
    RETRY_MAX = 7  # magic number

    # 3) Bare except and missing docstring (Rules 4, 5)
    def insecure_parse(data):
        try:
            return base64.b64decode(data)
        except:
            return None

    # 4) Deep nesting and inefficient processing (Rules 11, 12)
    def nested_and_slow(values: list) -> int:
        if values:
            if len(values) > 0:
                total = 0
                for i in range(len(values)):
                    for j in range(i + 1, len(values)):
                        if values[i] == values[j]:
                            total += 1
                if total >  10:  # inconsistent spacing
                    if total > 20:
                        if total > 30:
                            return total
                return total
    ...
    try:
        print(dangerous_shell("INJECTION_TEST"))
    except Exception as ex:
        print(f"Dangerous shell failed as expected: {ex}")
    print(insecure_parse("aGVsbG8="))
    print(nested_and_slow([1,2,3,1,2,3,1,2,3]))
